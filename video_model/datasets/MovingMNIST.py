...
class MovingMNIST(data.Dataset):
    ...
    def __init__(self, root, train=True, split=1000, transform=None, target_transform=None, download=False,image_size=32,seq_len = 20):
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            seq, target = self.train_data[index,:self.seq_len], {}
        else:
            seq, target = self.test_data[index,:self.seq_len], {}

        if self.transform is not None:
            seq = _transform_time(seq)
        if self.target_transform is not None:
            target = _transform_time(target)
        seq = (seq.type(torch.float32) / 127.5 - 1).unsqueeze(0)
