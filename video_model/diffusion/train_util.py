...
class TrainLoop:
    ...
    def run_loop(self):
        ...
        while (
            ...
            min_val = torch.min(batch) # normalization for the nturgbd skeleton dataset
            max_val = torch.max(batch)
            range_val = max_val - min_val
            batch = (batch - min_val) / range_val
            batch = batch * 2 - 1

            # min_val = torch.min(cond_batch)  # normalization for the nturgbd skeleton dataset
            # max_val = torch.max(cond_batch)
            # range_val = max_val - min_val
            # cond_batch = (cond_batch - min_val) / range_val
            # cond_batch = cond_batch * 2 - 1
    ...
    def forward_backward(self, batch):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            # [B, C, T, W, H]
            """
            uncomment for CLEVRER
            """
            micro=micro.permute(0,2,1,3,4)

            # micro=micro.permute(0,4,1,2,3) # uncomment for something-something and nturgbd skeletondataset
            # micro = (micro - micro.min()) / (micro.max() - micro.min())

            # breakpoint()

            idx = torch.randperm(micro.shape[2]) # random frames of 5
            idx=idx[idx!=0]
            idx=torch.cat((torch.tensor([0]),idx))
            # breakpoint()

            micro_cond= micro[:,:,idx,:,:] # shuffled video
            micro = micro_cond.clone()
            # print('training mode')
            breakpoint()

            # code for removing any frame
            # number of frames to remove
            num_frames_to_remove = 6
            # #
            # # # create an array of indices for all frames
            frame_indices = np.arange(micro_cond.shape[2])
            #
            ...
            dummy_frames = np.zeros(
                (micro_cond.shape[0], micro_cond.shape[1], 1, micro_cond.shape[3], micro_cond.shape[4]))
            dummy_frames = torch.from_numpy(dummy_frames).to(micro_cond.to(dist_util.dev()))          #
            #
            # # replace the selected frames with the dummy frames for all batches
            # breakpoint()
            micro_cond[:, :, remove_indices, :, :] = dummy_frames


            p_enc_1d_model = PositionalEncoding1D(16)
            time_emb = p_enc_1d_model(torch.rand(micro.shape[0], micro.shape[2], 16))
            time_emb = time_emb[:,idx,:]
            last_batch = (i + self.microbatch) >= batch.shape[0]
            t, weights = self.schedule_sampler.sample(micro.shape[0], dist_util.dev())
            # breakpoint()
            mask = np.ones(micro.shape[2])
            mask[remove_indices] = 0
            mask = torch.tensor(mask)
            compute_losses = functools.partial(
                self.diffusion.training_losses_two_dis,
                self.ddp_model,
                micro,
                t,
                condition_data=micro_cond,#model_kwargs
                time_emb=time_emb.to(dist_util.dev()),
